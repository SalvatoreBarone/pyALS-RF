...
class GREP:
    ...
    def split_test_dataset(self, pruning_set_fraction : float = 0.5):
        ...
        self.x_pruning, self.x_validation, self.y_pruning, self.y_validation, self.idx_prun, self.idx_test = train_test_split(self.classifier.x_test, self.classifier.y_test, indexes, train_size = pruning_portion, stratify = self.y_test.ravel()) # Stratify ensures that all classess are considered. 
