...
class SingleStepAlsHdlGenerator(HDLGenerator):
    ...
    def generate_axhdl(self, **kwargs):
        ...
        for i, conf in enumerate(kwargs['configurations']):
            ...
            chunks = []
            count = 0
            for size in [len(t.get_graph().get_cells()) for t in self.classifier.trees]:
                chunks.append([conf[i+count] for i in range(size)])
                count += size
            for t, c in zip(self.classifier.trees, chunks):
